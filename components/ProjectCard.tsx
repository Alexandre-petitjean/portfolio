...
export function ProjectCard({ 
  ...
  const getTechColor = (tech: string) => {
    return techColorMap[tech] || 'bg-gray-500/20 text-gray-700 dark:text-gray-300 border-gray-400/30'
  }
    ...
    <div className="group relative bg-gradient-to-br from-white/70 to-gray-50/70 dark:from-gray-800/50 dark:to-gray-900/50 backdrop-blur-sm border border-primary-500/20 rounded-3xl p-8 hover:border-primary-500/40 dark:hover:border-primary-400/40 transition-all duration-300 hover:transform hover:scale-[1.02] h-full shadow-sm">
      ...
      <div className="relative h-full flex flex-col">
        ...
        <div className="flex items-center space-x-3 mb-6">
          {/* Icon */}
          <div className={`w-12 h-12 bg-gradient-to-br ${colorMap[iconColor].bg} rounded-2xl flex items-center justify-center ${colorMap[iconColor].hoverBg} transition-all duration-300`}>
            <svg className={`w-6 h-6 ${colorMap[iconColor].text} transition-colors`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d={iconPath}/>
            </svg>
          </div>
          <h3 className={`text-xl font-bold text-gray-900 dark:text-white ${colorMap[iconColor].text} transition-colors`}>
            {title}
          </h3>
        </div>
        ...
        <div className="flex-grow">
          ...
          {successMetric && (
            <div className={`mb-6 p-4 ${getSuccessMetricClasses(successMetric.color).bg} rounded-2xl border ${getSuccessMetricClasses(successMetric.color).border}`}>
              <div className="flex items-center space-x-2">
                <svg className={`w-5 h-5 ${getSuccessMetricClasses(successMetric.color).iconText}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d={successMetric.icon}/>
                </svg>
                <p className={`${getSuccessMetricClasses(successMetric.color).text} font-semibold text-sm`}>{successMetric.text}</p>
              </div>
            </div>
          )}
          ...
          <div className="mb-6">
            <div className="flex flex-wrap gap-2">
              {technologies.map((tech, index) => (
                <span key={index} className={`px-3 py-1 ${getTechColor(tech)} text-xs font-medium rounded-full border`}>
                  {tech}
                </span>
              ))}
            </div>
        ...
        <div className="flex items-center justify-between mt-auto pt-4 border-t border-gray-300/30 dark:border-gray-700/30">
          <div className="flex items-center space-x-2 text-gray-600 dark:text-gray-400">
            ...
            <span className="text-sm">{isCompleted ? 'Terminé' : 'En cours'}</span>
          </div>
          {link && (
            <a href={link} className="text-primary-600 dark:text-primary-400 hover:text-primary-700 dark:hover:text-primary-300 transition-colors group/btn">
