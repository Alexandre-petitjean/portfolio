const messages = {
  "hero.available": "Disponible pour nouveaux projets",
  "hero.primaryTitle": "Expert",
  "hero.secondaryTitle": "Django & DevOps",
  "hero.description":
    "Développeur fullstack indépendant, je conçois et maintiens des applications critiques prêtes pour la production avec une approche DevOps moderne.",
  "hero.contact": "Discutons de votre projet",
  "hero.projects": "Voir mes réalisations",
  "hero.stats.years": "Années d'expérience",
  "hero.stats.projects": "Projets livrés",
  "hero.stats.satisfaction": "Satisfaction client",
  "terminal.situation.command": "$ situation",
  "terminal.situation.result":
    "> Deadline critique. Tech legacy. Stack bancale.",
  "terminal.action.command": "$ action",
  "terminal.action.result":
    "> Audit, refacto ciblé, CI/CD robuste, infra stable.",
  "terminal.outcome.command": "$ outcome",
  "terminal.outcome.result":
    "> Livraison à l'heure. Équipe libérée. Business sécurisé.",
  "terminal.feedback.command": "$ feedback",
  "terminal.feedback.result":
    '> "Alexandre a sauvé notre sprint. Et notre budget." – un CTO rassuré',
  "terminal.current_status.command": "$ current_status",
  "terminal.current_status.result": "✅ Disponible pour projets critiques.",
  "languageSwitcher.ariaLabel": "Changer la langue",
  "nav.home": "Accueil",
  "nav.services": "Services",
  "nav.skills": "Compétences",
  "nav.projects": "Projets",
  "nav.about": "À propos",
  "nav.contact": "Contact",
  "services.badge": "Services",
  "services.title": "Ce que je ",
  "services.description": "propose",
  "services.subtitle":
    "Des solutions techniques adaptées à vos enjeux business avec une expertise Django et DevOps",
  "services.django.title": "Développement Django",
  "services.django.description":
    "Applications web robustes, APIs REST performantes et intégrations complexes",
  "services.devops.title": "Solutions DevOps",
  "services.devops.description":
    "Automatisation d'infrastructure, pipelines CI/CD et optimisation cloud",
  "services.consulting.title": "Conseil technique",
  "services.consulting.description":
    "Conseils stratégiques pour aligner technologie et objectifs business",
  "services.cto.title": "CTO as a Service",
  "services.cto.description":
    "Accompagnement technique et stratégique pour structurer vos équipes",
  "services.approach.title": "Mon approche",
  "services.approach.description":
    "Je peux travailler seul ou en collaboration avec vos équipes existantes. Mon objectif est toujours le même : livrer du code robuste, compréhensible, bien intégré dans vos processus métier et technique.",
};
export default messages;
